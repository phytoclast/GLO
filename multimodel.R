...
sum(train$pos)/nrow(train)
  ...
  rf.prediction <-  predict(vars90, rf, na.rm=T);  names(prediction) <- taxon
  gm.prediction <-  predict(vars90, gm, na.rm=T, type = "response");  names(prediction) <- taxon
  mxnt.prediction <-  predict(vars90, mxnt, na.rm=T, type='logistic');  names(prediction) <- taxon
