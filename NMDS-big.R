...
MLRA94 <- subset(MLRA, MLRA %in% c("94A","94C","96","97","98","99","110","111C","111B"), select="LRU")
plot(MLRA94)
MLRA94.vect <- vect(MLRA94)
spts <- terra::spatSample(MLRA94.vect, size = 5000)
spts.mlra <- st_as_sf(spts) |> st_drop_geometry()
spts.spp <- terra::extract(Species, spts)
spts.env <- terra::extract(vars90, spts)

spts.es <- cbind(spts.spp, spts.env[,-1], spts.mlra) |> subset(!is.na(pH50) & !is.na(Tsuga) & !is.na(Acer) & !is.na(d))
...
colnames(spts.es)
spts.es.spp <- spts.es |> select(all_of(names(Species)))
